...
{
    void *alloc = lazybsd_mmap(NULL, bytes, lazybsd_PROT_READ|lazybsd_PROT_WRITE, lazybsd_MAP_ANON|lazybsd_MAP_PRIVATE, -1, 0);
     ...
     int flags)
{
    ...
    if ((mem = lazybsd_realloc(addr, size)) == NULL)
        lazybsd_free(addr);
